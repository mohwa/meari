 ...
 * @param {function} callbacks A function which returns the promise
 ...
 * @param {function} callbacks A function which returns the boolean value
 * @param {number} [retryCount=0]
 * @param {number} [delayTime=0]
 * @returns {Promise}
 * @example
...
export function every(promises) {
  return new Promise(resolve => {
    _seq(promises).then(() => resolve(true));
  });
}
...
export function some(promises) {
  return new Promise(resolve => {
    _seqAll(promises).then(values => {
      const result = !values.some(v => v.status === STATUS.REJECTED);
